...
import { useUpdateTaskDates } from "../hooks/useUpdateTaskDates"
...
export default function TaskDateCellPopover({
    ...
    const disabledDays: { before: Date; after: Date } | undefined =
        projectStart && projectEnd
            ? { before: projectStart, after: projectEnd }
            : projectStart
                ? { before: projectStart, after: new Date(2099, 11, 31) }
                : projectEnd
                    ? { before: new Date(1970, 0, 1), after: projectEnd }
                    : undefined
    ...
    return (
        <Popover>
            {() => (
                <>
                    <PopoverButton
                        className={`text-left text-xs transition-colors rounded px-1 py-0.5 -ml-1
                            ${hasDates
                            ...
                            <div className="space-y-2">
                                ...
                                {(projectStart || projectEnd) && (
                                    <p className="text-[10px] text-slate-400 italic">
                                        Rango del proyecto:{" "}
                                        {projectStart ? format(projectStart, "d MMM yyyy") : "—"}
                                        {" → "}
                                        {projectEnd ? format(projectEnd, "d MMM yyyy") : "—"}
                                    </p>
                                )}
                            </div>
