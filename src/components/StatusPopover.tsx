...
import { projectStatusTranslation, projectStatusStyles } from "../traductor/es"
import { CheckIcon } from "@heroicons/react/20/solid"
import type { ProjectStatus } from "../types"

type StatusPopoverProps = {
    status: ProjectStatus
    onSelect: (value: ProjectStatus) => void
    isPending?: boolean
}

const OPTIONS: { value: ProjectStatus; dotColor: string; hoverBg: string; textColor: string }[] = [
    { value: "planning", dotColor: "bg-purple-500", hoverBg: "hover:bg-purple-50", textColor: "text-purple-700" },
    { value: "active", dotColor: "bg-blue-500", hoverBg: "hover:bg-blue-50", textColor: "text-blue-700" },
    ...
    { value: "cancelled", dotColor: "bg-gray-500", hoverBg: "hover:bg-gray-50", textColor: "text-gray-600" },
]

export default function StatusPopover({ status, onSelect, isPending = false }: StatusPopoverProps) {
    const triggerStyle = projectStatusStyles[status]

    const handleSelect = (value: ProjectStatus, close: () => void, e?: React.MouseEvent) => {
        e?.stopPropagation()
        onSelect(value)
        close()
    }
            ...
            {({ close }) => (
                <>
                    <PopoverButton
                        ...
                        `}
                    >
                        {projectStatusTranslation[status]}
                    </PopoverButton>
                    <PopoverPanel
                        anchor="bottom start"
                            ...
                            {OPTIONS.map((opt) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {projectStatusTranslation[opt.value]}
                                        {isSelected && (
